...
\begin{itemize}
    ...
    \item \textbf{EFT cutoff:} $\Lambda_{\mathrm{QCT}} = 107$~TeV konzistentní s anomálním magnetickým momentem mionu
\end{itemize}
...
\textbf{\Cref{chap:eft}} formuluje QCT jako efektivní teorii pole s~UV cutoff $\Lambda_{\mathrm{QCT}} = 107$~TeV, včetně EFT operátorů a~jejich fenomenologických důsledků pro muon $g-2$.
...
\textbf{Diskrepance a současný status:}
...
\rowcolor{yellow!10}
Vazebná energie páru & $E_{\text{pair}}$ & \unit{eV} & $\mathbf{5{,}38 \times 10^{18}}$ \textbf{(CALIBR.)} \\
\rowcolor{yellow!10}
Efektivní hustota párů & $\rho_{\text{eff}}^{(\text{pairs})}$ & \unit{GeV^4} & $\mathbf{1{,}39 \times 10^{-29}}$ \\
Projekční poloměr (kosmický) & $R_{\text{proj}}^{(0)}$ & \unit{GeV^{-1}} & $2{,}3$--$2{,}6\,\unit{cm}$ \textbf{(DERIVED)} \\
Stínicí faktor & $f_{\text{screen}}$ & -- & $m_\nu/m_p \sim 10^{-10}$ \textbf{(DERIVED)} \\
\rowcolor{yellow!10}
Fázová variance (saturovaná) & $\sigma_{\max}^2$ & -- & $\mathbf{0{,}2}$ \textbf{(FITTED)} \\
\rowcolor{yellow!10}
Astrofyzikální $G_{\text{eff}}/G_N$ & -- & -- & $\mathbf{\sim 0{,}9}$ \textbf{(DERIVED)} \\
...
\Lambda_{\text{QCT}} &= 107 \, \unit{TeV} = 1{,}07 \times 10^5 \, \unit{GeV} \\
\alpha_0 &\sim 0{,}1
\end{align}
...
QCT je formulována jako \textbf{efektivní teorie pole} (EFT) platná do energetické škály $\Lambda_{\text{QCT}} \sim 107\,\unit{TeV}$. V~této kapitole prezentujeme kompletní lagrangián, Wilsonovy koeficienty a spojení s~akustickou metrikou z~analogové gravitace.
...
kde $\Lambda_{\text{QCT}} = 107\,\unit{TeV}$ je cutoff škála (z~muon $g-2$), $\Delta_i$ je hmotnostní dimenze operátoru $\mathcal{O}_i$ a $c_i$ jsou Wilsonovy koeficienty.
...
\item \textbf{EFT operátory:} Dimenze 6 operátory s~cutoffem $\Lambda_{\text{QCT}} = 107\,\unit{TeV}$
...
\item Cutoff $\Lambda_{\text{QCT}} = 107\,\unit{TeV}$ z~muon $g-2$
...
\textbf{Numerická verifikace:} Pro dnešní $E_{\mathrm{pair}}(0)\approx 10^{19}$~eV:
\begin{equation}
\Lambda_{\mathrm{QCT}}(0) = \frac{3}{2}\sqrt{10^{19}\,\unit{eV}\times 9{,}38\times 10^{8}\,\unit{eV}} = \frac{3}{2}\times 96{,}6\,\unit{TeV} = 107\,\unit{TeV}.
\end{equation}
...
\Lambda_{\mathrm{baryon}} &= \sqrt{E_{\mathrm{pair}}\cdot m_p} \sim 71\,\unit{TeV} \quad \text{(vazba s~baryony)}, \\
\Lambda_{\mathrm{QCT}} &= \tfrac{3}{2}\Lambda_{\mathrm{baryon}} \sim 107\,\unit{TeV} \quad \text{(efektivní EFT škála)}.
\end{align}
...
\end{equation}
Dnes (z=0): $\Lambda=107$~TeV. Při rekombinaci (z$\sim$1100): $\Lambda\approx 84$~TeV. Při BBN (z$\sim 10^{9}$): $\Lambda\approx 107$~TeV.
...
\begin{itemize}
\item $\Lambda_{\mathrm{QCT}} = 107$~TeV
...
$\Lambda_{\mathrm{QCT}}$ & $107$~TeV (odvozeno) & --- & faktor 3/2 z~flavor \\
...
\textbf{Status:} SPLNĚN (lokálně, při energiích $E \ll \Lambda_{\mathrm{QCT}} \sim 100$~TeV)
...
$\Lambda_{\rm QCT}$ & $107$~TeV & Muon $g-2$ \\
...
% ============================================================================
% Nové appendixy - řešení identifikovaných problémů
% ============================================================================
