...
\begin{align}
\text{Hluboký vesmír } (\Phi \approx 0): &\quad \lambda_{\text{screen}}^{(0)} \approx 1{,}0\,\unit{mm} \\
\text{Povrch Země:} &\quad \lambda_{\text{screen}}^\oplus \approx 40\,\unit{\mu m} \\
\text{Oběžná dráha ISS:} &\quad \lambda_{\text{screen}}^{\text{ISS}} \approx 41\,\unit{\mu m}
\end{align}
...
\item \textbf{Klíčový test:} Sub-mm měření na ISS vs. Zemi by měla ukázat rozdíl $\sim 2{,}5\,\%$ v~$\lambda_{\text{screen}}$
\end{enumerate}
