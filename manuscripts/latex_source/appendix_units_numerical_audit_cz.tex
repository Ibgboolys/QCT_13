...
\begin{itemize}
\item Predikce pro Zemi: $\lambda_{\rm screen}^\oplus = 40\,\mu\mathrm{m}$ — \emph{perfektní shoda s limitem Eöt-Wash!}
\item ISS vs. Země: $41\,\mu\mathrm{m}$ vs. $40\,\mu\mathrm{m}$ (2.5\%) rozdíl — testovatelné!
\item Původní predikce v5.1 $\lambda \sim 1$ mm platí pouze ve vakuu vesmíru.
\end{itemize}
...
\textbf{Klíčové změny:}
\begin{enumerate}
\item \textbf{\(m_\nu \to m_p\):} Škála odseknutí zahrnuje vazbu s baryonovým prostředím, nejen mikroskopickou škálu kondenzátu.
\item \textbf{Faktor 3/2:} Pochází z průměrování přes tři příchutě neutrin \((\nu_e, \nu_\mu, \nu_\tau)\).
\item \textbf{Numerická verifikace:} S \(E_{\rm pair}=5.38\times10^9\,\mathrm{GeV}\) a \(m_p=0.938\,\mathrm{GeV}\):
\begin{equation}
