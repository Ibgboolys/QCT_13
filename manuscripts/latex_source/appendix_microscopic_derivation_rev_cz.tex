...
\paragraph{Počáteční párovací energie $E_0$.}

V~okamžiku odpojení je minimální energie pro neutrinové párování nastavena škálou klidové hmotnosti:
\begin{equation}
E_0 = m_\nu c^2 \approx 0.1 \, {\rm eV}
\label{eq:E0_natural}
\end{equation}

To \emph{není} libovolné — je to přirozená energetická škála pro nerelativistické neutrinové páry.

\paragraph{Konstanta uzavření $\kappa_{\rm conf}$.}

Rychlost růstu párovací energie je určena silou uzavření. Ze současné fenomenologie QCT (fitování na $E_{\rm pair}(z=0) \sim 10^{19}$ eV):
\begin{equation}
