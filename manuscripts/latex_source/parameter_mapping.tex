...
\begin{align}
G_{\rm eff} &= 1 \times \frac{(6\times 10^{-9}\,{\rm kg/m}^3)(72.3\times 10^{-6}\,{\rm m}^3)}{2.58\times 10^{-2}\,{\rm m}} \\
...
116.9\,{\rm TeV} = 1.169\times 10^5\,{\rm GeV}$, which is the scale of new physics (non-perturbative RG jumps). This value is \textbf{not a free parameter} but is fixed by the see-saw relation (Appendix~\ref{app:proton_mass_decomposition}):
\begin{equation}
\Lambda_{\rm QCT} = \frac{\Lambda_\mu^2}{\sqrt{\sigma_{\rm QCD}}} = \frac{(518.6\,{\rm MeV})^2}{420\,{\rm MeV}} = 116.9\,{\rm TeV}
\end{equation}
This represents a topological UV cutoff determined by proton mass decomposition.
...
\begin{equation}
K_{\rm cond} = P_{\rm vac} = \rho_{\rm ent}\,c^2 \approx 9.4\times 10^{56}\,{\rm Pa}.
\end{equation}
\textbf{Note:} This updated value reflects the correct vacuum pressure derived from the see-saw relation (Appendix~\ref{app:proton_mass_decomposition}). The earlier estimate ($5.4\times 10^8$ Pa) was based on preliminary calculations and has been superseded by the rigorous proton mass decomposition, which fixes $P_{\rm vac}$ via string tension $\sigma_{\rm QCD}$.
...
\begin{equation}
\alpha_{\rm em} \sim \frac{(1.6\times 10^{-19}\,{\rm C})^2/(4\pi\epsilon_0\cdot 2.58\times 10^{-2}\,{\rm m})}{(6\times 10^{-9}\,{\rm kg/m}^3)(7.2\times 10^{-5}\,{\rm m}^3)(c^2)} \sim ?
\end{equation}
...
\begin{table}[h]
\centering
\caption{Parameter Conversion Table (Updated with v2.0 Values).}
\begin{tabular}{llll}
\toprule
...
$P_{\rm vac} = K_{\rm cond}$ & run $\alpha(\mu)$ & \eqref{eq:c_stiffness} & stiffness $9.4\times 10^{56}$ Pa \\
$\Lambda_{\rm micro} = 518.6$ MeV & $\Lambda_{\rm QCT} = 116.9$ TeV & See-saw: $\Lambda_{\rm QCT} = \Lambda_\mu^2/\sqrt{\sigma}$ & Topological UV cutoff \\
$\sqrt{\sigma_{\rm QCD}} = 420$ MeV & Confinement energy & Flux tube tension & Shell energy (proton) \\
$\partial_\mu\theta$ & $A_\mu$ (photon) & Goldstone mode & gauge field \\
Topol. vortex & charge $q=ne$ & quantization & $q\sim n\oint\nabla\theta$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{New Additions (v2.0):}
\begin{itemize}
\item \textbf{Proton mass decomposition:} $m_p = \Lambda_\mu + \sqrt{\sigma_{\rm QCD}} = 518.6 + 420 = 938.6$ MeV (App.~\ref{app:proton_mass_decomposition})
\item \textbf{See-saw relation:} $\Lambda_{\rm QCT} = \Lambda_\mu^2 / \sqrt{\sigma_{\rm QCD}}$ fixes UV cutoff
\item \textbf{Vacuum stiffness:} $P_{\rm vac} = 9.4\times 10^{56}$ Pa (updated from preliminary $5.4\times 10^8$ Pa)
\item \textbf{Triple Lock mechanism:} Gravity turn-on at $z \approx 1100$ (App.~\ref{app:triple_lock_cosmology})
\item \textbf{Void-Driven Dark Energy:} Late-time acceleration from structure formation (App.~\ref{app:void_driven_dark_energy})
\end{itemize}
