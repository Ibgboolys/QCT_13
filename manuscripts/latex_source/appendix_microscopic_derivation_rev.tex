...
\subsubsection{Time Dependence of $E_{\rm pair}$ -- Historical Model (DEPRECATED)}

\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=⚠️ Deprecated Paradigm]
The following model represents the \textbf{original phenomenological approach} (2020--2024), which has been superseded by the primordial freezeout paradigm.

\textbf{Current paradigm (2025):} $E_{\mathrm{cond}} = 2 \times 10^{16}$ GeV is a fixed constant from the GUT epoch (see Section~7.3 in main text).

This section is preserved for documentation of theoretical development.
\end{tcolorbox}

The pairing energy in the \emph{original model} evolved cosmologically as:
\begin{equation}
E_{\rm pair}(z) = E_0 + \kappa_{\rm conf} \cdot f_{\rm turn-on}(z, z_{\rm start}) \cdot \ln(1+z) \quad \text{(DEPRECATED)}
\label{eq:Epair_evolution}
\end{equation}

where the turn-on function was:
\begin{equation}
...
with steepness parameter $k \sim 2$. This sigmoid function ensured smooth transition:
\begin{align}
...
\paragraph{Initial Pairing Energy $E_0$ (historical model).}

In the \emph{original model}, at the moment of decoupling, the minimal energy for neutrino pairing was set by the rest mass scale:
\begin{equation}
E_0 = m_\nu c^2 \approx 0.1 \, {\rm eV} \quad \text{(DEPRECATED)}
\label{eq:E0_natural}
\end{equation}

\paragraph{Confinement Constant $\kappa_{\rm conf}$ (historical model).}

In the \emph{original model}, the growth rate of pairing energy was determined by the confinement strength. From QCT phenomenology (fitting to $E_{\rm pair}(z=0) \sim 10^{19}$ eV):
\begin{equation}
\kappa_{\rm conf} \approx 4.8 \times 10^{17} \, {\rm eV} = 0.48 \, {\rm EeV} \quad \text{(DEPRECATED)}
\label{eq:kappa_conf_value}
\end{equation}

\textbf{In the current paradigm} these parameters are not needed, as $E_{\mathrm{cond}}$ is a fixed constant.
